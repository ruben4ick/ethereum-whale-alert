...
import (
	"context"
	"errors"
	bloom2 "ethereum-whale-alert/internal/benchmarks/bloombench"
	"log/slog"
	"math/big"
	"math/rand"
	"os"
...
type Config struct {
	...
	OutputPrefix      string  `envconfig:"BENCH_OUT" default:"./benchresults/bloombench"`
}
...
func main() {
	...
	topics := bloom2.DefaultTopics()
	runner := bloom2.New(adapter, bloom2.Config{
		BlockCount:  cfg.BlockCount,
		Concurrency: cfg.Concurrency,
		Topics:      topics,
	})

	slog.Info("starting bloombench bench",
	...
	if err := bloom2.WriteCSV(csvPath, results, topics); err != nil {
		slog.Error("write csv", "error", err)
		os.Exit(1)
	}

	summary := bloom2.BuildSummary(results, topics)
	if err := bloom2.WriteJSON(jsonPath, summary); err != nil {
		slog.Error("write json", "error", err)
		os.Exit(1)
	}

	bloom2.PrintSummary(summary)
